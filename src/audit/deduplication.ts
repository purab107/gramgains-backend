 ...
 */
export class DeduplicationAuditor {
  private barcodeMap = new Map<string, Array<{ id: string; name: string; source: string }>>();
  private exactNameMap = new Map<string, Array<{ id: string; name: string; brand: string | null; source: string; layer: number }>>();
  private blockBuckets = new Map<string, Array<{ id: string; name: string; tokens: string[]; source: string; layer: number; category: string }>>();

  /**
   * Registers a food record into the deduplication indexes.
   */
  public registerFood(food: FoodRecord): void {
   ...
   */
  public generateIssues(similarityThreshold = 0.85): AuditIssue[] {
